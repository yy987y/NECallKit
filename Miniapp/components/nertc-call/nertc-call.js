import YunXinMiniappSDK from './resources/sdk/NERTC_Miniapp_SDK_for_WeChat_v4.2.1'
...
import { requestId, toast } from './utils/index'
...
Component({
  ...
  methods: {
    ...
    _keepChannelOn() {
      ...
      this.channelTimer = setInterval(() => {
        if (this.channelInfo?.channelId) {
          console.warn("===signalingDelay", this.channelInfo)
    ...
    */
    setupAppKey({ debug, appkey }) {
      logger.log(TAG_NAME, 'setupAppKey')
    ...
    */
    login({ account, token, ...opt }) {
      console.log("===login", this.data.config)
      const _this = this
  ...
  */
    ...
    async call({ userId, type, success, fail, attachment }) {
      console.log("this.callStatus", this.callStatus, attachment)
      try {
        if (!this.NIM) {
          throw SIGNAL_NULL;
        }
        ...
        console.warn("独立呼叫信令，邀请别人加入频道成功，data：", this.channelInfo)
        success?.({ ...this.channelInfo, ...this.joinInfo.members?.[0] });
        console.log("====this.callTimeout", this.callTimeout, this.callStatus)
    ...
    */
    ...
    _signalMsg() {
      ...
      this.NIM.on('signalingNotify', async (event) => {
        console.log("signalingOnlineNotify: ", event, this.mutiChannelId)
        if (this.mutiChannelId == event.channelId) {
          console.warn("远端设备的消息this.mutiChannelId", this.mutiChannelId)
          return
        }
        switch (event.eventType) {
          case 'ROOM_CLOSE':
            ...
            if (this.mutiClientOp) {
              console.warn("this.mutiClientOp")
              return
            }
            if (this.channelInfo.channelId !== event.channelId) {
              console.warn("channleId不同")
              return
            }
          ...
          case 'CANCEL_INVITE':
            ...
            this._sendMessage(event.to, 'canceled');
            this.callStatus = CallingStatus.idle
            break;
          case 'REJECT':
            console.log("独立呼叫信令：拒绝邀请事件", event);
            if (this.channelInfo.channelId !== event.channelId) {
              console.warn("channleId不同")
              return
            }
          ...
          case 'CONTROL':
            ...
            if (attachExt.cid === 1) {
              ...
              console.log("==info", info, member)
              this._enterRoom({ channelName: event.channelId, uid: member[0]?.uid })
              this.durations = [
                {
    ...
    async _signalOfflineMsg() {
      ...
      this.NIM.on(
        ...
        async (data) => {
          console.warn("signalingUnreadMessageSyncNotify", data)
    ...
    async _inviteHandler(event) {
      ...
      this._emitter.emit("onInvited", {
        ...event,
        inviter: event.creator,
        userIds: [],
        isFromGroup: false,
        groupId: null,
        type: event.type,
        attachment: event.attachExt
      })
      // 被叫超时后，如果还是called状态，会自动拒绝
      if (this.rejectTimeout !== undefined) {
        console.log("==this.rejectTimer", this.callStatus)
    ...
    //被邀请者接受
    async accept(params) {
      console.log("accept", params, this.channelInfo)
      this._clearTimer();
      try {
        const param = {
        ...
        this.NIM.signalingAccept(param).then(data => {
        ...
        this.client.join(params)
          ...
          .then(url => {
            ...
            let func = () => {
              ...
              console.warn("===this.data.pusher??", this.data.pusher)
              this.setData({
                pusher
              }, () => {
          ...
          .catch(error => {
            ...
            console.warn("'enterRoom fail:", error)
            reject(error)
          })
      })
    },
    ...
    */
    _exitRoom(needBack = true) {
      // console.warn("===_exitRoom", this.status.hasExit, needBack)
      ...
      this.client.leave().then(() => {
        ...
        this._resetState()
        console.warn("'exitRoom fail: ", err)
      })
      if (needBack) {
        wx.navigateBack({
          delta: 1,
    ...
    _muteLocalVideo() {
      return this.client.mute('video').then(url => {
        console.log('mute 视频成功: ', url)
        // return this._setPusherConfig({ url })
      }).catch(e => {
        console.error('mute 视频失败: ', e)
      })
    },
    _unmuteLocalVideo() {
      return this.client.unmute('video').then(url => {
        console.log('unmute 视频成功: ', url)
        // return this._setPusherConfig({ url })
      }).catch(e => {
        console.error('unmute 视频失败: ', e)
      })
    },

    _muteLocalAudio() {
      return this.client.mute('audio').then(url => {
        console.log('mute 音频成功: ', url)
        return this._setPusherConfig({ url })
      }).catch(e => {
    ...
    _unmuteLocalAudio() {
      return this.client.unmute('audio').then(url => {
        console.log('unmute 音频成功: ', url)
        return this._setPusherConfig({ url })
      }).catch(e => {
        console.error('unmute 音频失败: ', e)
      })
    },
    /**
     * 开启麦克风
    ...
    _getChannelMember(channelName) {
      return new Promise((resolve, reject) => {
        this.NIM.signalingGetChannelInfo({
          ...
          console.warn("_getChannelMember err", err)
          resolve({})
        });
      })
    ...
    */
    _sendMessage(userId, status) {
      console.log("====begin send msg===", userId, this.durations, this.durations.map((item) => ({
        ...item,
    ...
    _setPusherConfig(config) {
      return new Promise((resolve) => {
        let pusher
        console.log("====_setPusherConfig", this.data.pusher)
        if (!this.data.pusher) {
    ...
    _setList() {
      console.log("====_setList", this.userController.getAllUser())
      return new Promise(resolve => {
        this.setData({
    ...
    _resetState() {
      console.warn("===_resetState", this.data, this)
      if (this.data.pusher) {
    ...
    _onSyncDone(data) {
      ...
      console.warn("_onSyncDone", data, this.data.config.uid)
    ...
    _playerStateChange(event) {
      ...
      console.warn("===_playerStateChange", event)
      switch (code) {
        case -2301:
    ...
    _playerNetStatus(event) {
      ...
      // console.warn("_playerNetStatus", event)
      if (this.data.statusVisible) {
    ...
    async _hangUp(sendMsg) {
      try {
        console.log("--hangUp", this.channelInfo, this.callStatus)
        if (this.userId) {
          await this.cancel({
            channelId: this.channelInfo.channelId,
            account: this.userId,
